 ...
 *
 *   m.library.add('template', function () {
 *     return new Template();
 *   };
 */
(function (m, _) {
  var Library = m.create({
    ...
    add: function (name, factory) {
      if (this.registry[name]) {
        throw new Error('Library already contains an item with name: ' + name);
      }
    ...
    /* Checks to see if the Library contains the specified module. This is
    ...
    require: function (dependencies) {
      return new ModuleDependencies(this, dependencies);
    }
  });
   ...
   * object provided.
   */
  var ModuleDependencies = m.create({
    instances: null,
    dependencies: null,
    library: null,

    constructor: function ModuleDependencies(library, dependencies) {
      this.instances = {};
     ...
     */
    teardown: function () {
      _.each(this.instances, function (name, instance) {
  ...
  /* Export the Library constructor and a new instance */
  m.Library = Library;
  m.library = new Library();
